...
const pathr_proxy_contract_address_1 = require("../../../../../../cross-chain/calculation-manager/providers/common/constants/pathr-proxy-contract-address");
...
const gateway_pathr_cross_chain_abi_1 = require("../../../../../../cross-chain/calculation-manager/providers/common/emv-cross-chain-trade/constants/gateway-pathr-cross-chain-abi");
...
class EvmOnChainTrade extends on_chain_trade_1.OnChainTrade {
    ...
    async approve(options, checkNeedApprove = true, amount = 'infinity') {
        ...
        const approveAmount = this.from.blockchain === blockchain_name_1.BLOCKCHAIN_NAME.GNOSIS ||
            ...
            ? this.from.weiAmount
            : amount;
        return this.web3Private.approveTokens(this.from.address, this.spenderAddress, approveAmount, options);
    }
    ...
    async getProxyContractParams(options) {
        ...
        const proxyFee = new bignumber_js_1.default(this.feeInfo.pathrProxy?.fixedFee?.amount || '0');
        const value = web3_pure_1.Web3Pure.toWei(proxyFee.plus(this.from.isNative ? this.from.tokenAmount : '0'), nativeToken.decimals);
        const txConfig = evm_web3_pure_1.EvmWeb3Pure.encodeMethodCall(pathr_proxy_contract_address_1.pathrProxyContractAddress[this.from.blockchain].router, evm_common_cross_chain_abi_1.evmCommonCrossChainAbi, 'swapTokensGeneric', methodArguments, value);
    ...
    async getSwapData(options) {
        const directTransactionConfig = await this.encodeDirect({
            ...options,
            fromAddress: pathr_proxy_contract_address_1.pathrProxyContractAddress[this.from.blockchain].router,
            supportFee: false,
            receiverAddress: pathr_proxy_contract_address_1.pathrProxyContractAddress[this.from.blockchain].router
        });
