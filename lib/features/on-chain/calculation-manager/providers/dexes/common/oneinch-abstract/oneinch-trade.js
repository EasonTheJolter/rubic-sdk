...
class OneinchTrade extends evm_on_chain_trade_1.EvmOnChainTrade {
    ...
    static async checkIfNeedApproveAndThrowError(from, toToken, fromWithoutFee, fromAddress, useProxy) {
        ...
        if (needApprove) {
            throw new errors_1.PathrSdkError('Approve is needed');
        }
    }
    get type() {
    ...
    getTradeData(disableEstimate = false, fromAddress, receiverAddress) {
        ...
        return (0, utils_1.oneInchHttpGetRequest)('swap', this.from.blockchain, swapRequest);
    }
    specifyError(err) {
        ...
        if (inchError) {
            if ('message' in inchError) {
                if (inchError.message?.includes('cannot estimate')) {
                    ...
                    return new errors_1.PathrSdkError(message);
                }
