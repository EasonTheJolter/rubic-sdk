...
const oneinch_trade_1 = require("src/features/on-chain/calculation-manager/providers/dexes/common/oneinch-abstract/oneinch-trade");
const utils_1 = require("src/features/on-chain/calculation-manager/providers/dexes/common/oneinch-abstract/utils");
class OneinchAbstractProvider extends evm_on_chain_provider_1.EvmOnChainProvider {
    ...
    async getTradeInfo(from, toToken, fromWithoutFee, options) {
        ...
        try {
            ...
            if (oneInchTrade.hasOwnProperty('errors') || !oneInchTrade.toAmount) {
                throw new errors_1.PathrSdkError('1inch quote error');
            }
    ...
    async extractPath(fromToken, toToken, oneInchTrade) {
        const addressesPath = oneInchTrade.protocols[0].map(protocol => {
            if (!protocol?.[0]) {
                throw new errors_1.PathrSdkError('Protocol array must not be empty');
            }
        ...
        const tokensPath = addressesPath.map(tokenAddress => {
            ...
            if (!token) {
                throw new errors_1.PathrSdkError('Token has to be defined');
            }
