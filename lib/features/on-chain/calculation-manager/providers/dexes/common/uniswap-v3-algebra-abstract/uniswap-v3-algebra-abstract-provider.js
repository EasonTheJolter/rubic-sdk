...
const get_from_to_tokens_amounts_by_exact_1 = require("src/features/on-chain/calculation-manager/providers/dexes/common/utils/get-from-to-tokens-amounts-by-exact");
class UniswapV3AlgebraAbstractProvider extends evm_on_chain_provider_1.EvmOnChainProvider {
    constructor() {
        super(...arguments);
        this.isPathrOptimisationEnabled = false;
        this.defaultOptions = {
    ...
    async getRoute(from, to, exact, weiAmount, options, gasPriceInUsd) {
        ...
        if (this.isPathrOptimisationEnabled &&
            ...
            const calculatedProfits = routes.map((route, index) => {
                ...
                if (!estimatedGas) {
                    throw new errors_1.PathrSdkError('Estimated gas has have to be defined');
                }
            ...
            if (!sortedRoutes) {
                throw new errors_1.PathrSdkError('Sorted routes have to be defined');
            }
            return sortedRoutes;
        }
        const route = routes[0];
        if (!route) {
            throw new errors_1.PathrSdkError('Route has to be defined');
        }
