...
const izumi_trade_1 = require("src/features/on-chain/calculation-manager/providers/dexes/common/izumi-abstract/izumi-trade");
...
class IzumiProvider extends evm_on_chain_provider_1.EvmOnChainProvider {
    ...
    async calculate(from, to, options) {
        ...
        const tokenIn = {
            chainId,
            symbol: from.isNative ? tokens_1.wrappedNativeTokensList[this.blockchain]?.symbol : from.symbol,
            address: from.isNative ? wrapped_addresses_1.wrappedAddress[this.blockchain] : from.address,
            decimal: from.decimals
        };
        const tokenOut = {
            chainId,
            symbol: to.isNative ? tokens_1.wrappedNativeTokensList[this.blockchain]?.symbol : to.symbol,
        ...
        try {
            ...
            if (!pathQueryResult) {
                throw new errors_1.PathrSdkError('No result');
            }
        }
        catch (err) {
            console.debug(err);
        ...
        const wrapAddress = tokens_1.wrappedNativeTokensList[from.blockchain]?.address;
