...
const type_guards_1 = require("src/features/on-chain/calculation-manager/utils/type-guards");
class PathFactory {
    get walletAddress() {
    ...
    async getAmountAndPath(gasPriceInUsd) {
        ...
        if (this.options.gasCalculation === 'disabled') {
            if (!(0, type_guards_1.hasLengthAtLeast)(sortedRoutes, 1)) {
                throw new errors_1.PathrSdkError('Routes array length has to be bigger than 0');
            }
            return {
                route: sortedRoutes[0]
            };
        }
        if (this.options.gasCalculation === 'pathrOptimisation' &&
            ...
            const routesWithProfit = sortedRoutes.map((route, index) => {
                ...
                if (!estimatedGas) {
                    throw new errors_1.PathrSdkError('Estimated gas has to be defined');
                }
            ...
            if (!sortedByProfitRoutes?.[0]) {
                throw new errors_1.PathrSdkError('Profit routes array length has to be bigger than 0');
            }
        ...
        if (this.walletAddress) {
            ...
            if (!callData) {
                throw new errors_1.PathrSdkError('Call data has to be defined');
            }
        ...
        if (!sortedRoutes?.[0]) {
            throw new errors_1.PathrSdkError('Routes length has to be bigger than 0');
        }
        return {
            route: sortedRoutes[0],
    ...
    async getAllRoutes() {
        ...
        const tokens = responses.map((response, index) => {
            ...
            if (!numberAmount) {
                throw new errors_1.PathrSdkError('Amount has to be defined');
            }
            const outputAbsoluteAmount = new bignumber_js_1.default(numberAmount);
            const path = routesPaths?.[index];
            if (!path) {
                throw new errors_1.PathrSdkError('Path has to be defined');
            }
