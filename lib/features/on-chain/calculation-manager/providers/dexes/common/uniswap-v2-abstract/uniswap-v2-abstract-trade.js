...
const functions_1 = require("src/common/utils/functions");
...
const evm_on_chain_trade_1 = require("src/features/on-chain/calculation-manager/providers/common/on-chain-trade/evm-on-chain-trade/evm-on-chain-trade");
const default_estimated_gas_1 = require("src/features/on-chain/calculation-manager/providers/dexes/common/uniswap-v2-abstract/constants/default-estimated-gas");
const SWAP_METHOD_1 = require("src/features/on-chain/calculation-manager/providers/dexes/common/uniswap-v2-abstract/constants/SWAP_METHOD");
const uniswap_v2_abi_1 = require("src/features/on-chain/calculation-manager/providers/dexes/common/uniswap-v2-abstract/constants/uniswap-v2-abi");
class UniswapV2AbstractTrade extends evm_on_chain_trade_1.EvmOnChainTrade {
    ...
    static getDexContractAddress(blockchain) {
        ...
        if (!instance.dexContractAddress) {
            throw new errors_1.PathrSdkError('Trying to read abstract class field');
        }
        return instance.dexContractAddress;
    }
    static get type() {
        throw new errors_1.PathrSdkError(`Static TRADE_TYPE getter is not implemented by ${this.name}`);
    }
    ...
    async encodeDirect(options) {
        ...
        if (options.supportFee === undefined) {
            ...
            if (needApprove) {
                throw new errors_1.PathrSdkError('To use `encode` function, token must be approved for wallet');
            }
            try {
                await this.checkBalance();
            }
            catch (_err) {
                throw new errors_1.PathrSdkError('To use `encode` function, wallet must have enough balance or you must provider `supportFee` parameter in options.');
            }
        }
        try {
    ...
    getDefaultEstimatedGas() {
        ...
        if (!gasLimitAmount) {
            throw new errors_1.PathrSdkError('Gas limit has to be defined');
        }
