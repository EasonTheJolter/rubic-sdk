...
class CrossChainSymbiosisManager {
    ...
    async getUserTrades(fromAddress) {
        ...
        if (!fromAddress) {
            throw new errors_1.PathrSdkError('`fromAddress` parameter or wallet address must not be empty');
        }
    ...
    async revertTrade(revertTransactionHash, options = {}) {
        ...
        if (!stuckedTrade) {
            throw new errors_1.PathrSdkError('No request with provided transaction hash');
        }
