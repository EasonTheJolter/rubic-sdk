...
const tx_status_1 = require("src/core/blockchain/web3-public-service/web3-public/models/tx-status");
const injector_1 = require("src/core/injector/injector");
const changenow_api_key_1 = require("src/features/common/providers/changenow/constants/changenow-api-key");
const get_bridgers_trade_status_1 = require("src/features/common/status-manager/utils/get-bridgers-trade-status");
const get_src_tx_status_1 = require("src/features/common/status-manager/utils/get-src-tx-status");
const cross_chain_trade_type_1 = require("src/features/cross-chain/calculation-manager/models/cross-chain-trade-type");
const cbridge_cross_chain_api_service_1 = require("src/features/cross-chain/calculation-manager/providers/cbridge/cbridge-cross-chain-api-service");
const cbridge_status_response_1 = require("src/features/cross-chain/calculation-manager/providers/cbridge/models/cbridge-status-response");
...
const lifi_swap_status_1 = require("src/features/cross-chain/calculation-manager/providers/lifi-provider/models/lifi-swap-status");
...
const symbiosis_swap_status_1 = require("src/features/cross-chain/calculation-manager/providers/symbiosis-provider/models/symbiosis-swap-status");
...
const cross_chain_cbridge_manager_1 = require("src/features/cross-chain/cbridge-manager/cross-chain-cbridge-manager");
const multichain_status_mapping_1 = require("src/features/cross-chain/status-manager/constants/multichain-status-mapping");
const changenow_api_response_1 = require("src/features/cross-chain/status-manager/models/changenow-api-response");
const squidrouter_transfer_status_enum_1 = require("src/features/cross-chain/status-manager/models/squidrouter-transfer-status.enum");
const statuses_api_1 = require("src/features/cross-chain/status-manager/models/statuses-api");
...
class CrossChainStatusManager {
    ...
    async getDstTxData(srcTxStatus, tradeData, tradeType) {
        ...
        if (!getDstTxStatusFn) {
            throw new errors_1.PathrSdkError('Unsupported cross chain provider');
        }
    ...
    getBridgersDstSwapStatus(data) {
        if (!data.amountOutMin) {
            throw new errors_1.PathrSdkError('field amountOutMin is not set.');
        }
        return (0, get_bridgers_trade_status_1.getBridgersTradeStatus)(data.srcTxHash, data.fromBlockchain, 'pathr', data.amountOutMin);
    }
    /**
     * @internal
    ...
    async getChangenowDstSwapStatus(data) {
        if (!data.changenowId) {
            throw new errors_1.PathrSdkError('Must provide changenow trade id');
        }
        try {
    ...
    async getSquidrouterDstSwapStatus(data) {
        try {
            const { status, toChain } = await this.httpClient.get(`${squidrouter_cross_chain_provider_1.SquidrouterCrossChainProvider.apiEndpoint}status?transactionId=${data.srcTxHash}`, { headers: { 'x-integrator-id': 'pathr-api' } });
    ...
    async getTaikoBridgeDstSwapStatus(data) {
        if (!data.taikoTransactionId) {
            throw new errors_1.PathrSdkError('Must provide Taiko transaction ID');
        }
        if (!data.sender) {
            throw new errors_1.PathrSdkError('Must specify sender account');
        }
        ...
        if (!items[0]) {
            throw new errors_1.PathrSdkError('Taiko Relayer did not find transaction with such ID');
        }
