...
const cbridge_cross_chain_trade_1 = require("src/features/cross-chain/calculation-manager/providers/cbridge/cbridge-cross-chain-trade");
const cbridge_contract_abi_1 = require("src/features/cross-chain/calculation-manager/providers/cbridge/constants/cbridge-contract-abi");
const cbridge_contract_address_1 = require("src/features/cross-chain/calculation-manager/providers/cbridge/constants/cbridge-contract-address");
const cbridge_supported_blockchains_1 = require("src/features/cross-chain/calculation-manager/providers/cbridge/constants/cbridge-supported-blockchains");
...
class CbridgeCrossChainProvider extends cross_chain_provider_1.CrossChainProvider {
    ...
    async calculate(fromToken, toToken, options) {
        ...
        try {
            ...
            if (!config.supportedToToken) {
                throw new errors_1.PathrSdkError('To token is not supported');
            }
            ...
            if (!amount) {
                throw new errors_1.PathrSdkError('Can not estimate trade');
            }
    ...
    async fetchContractAddressAndCheckTokens(fromTokenOrNative, toTokenOrNative) {
        ...
        if (fromToken.isNative) {
            const wrappedFrom = tokens_1.wrappedNativeTokensList[fromTokenOrNative.blockchain];
        ...
        if (toToken.symbol === native_tokens_1.nativeTokensList[toToken.blockchain].symbol) {
            const wrappedTo = tokens_1.wrappedNativeTokensList[toTokenOrNative.blockchain];
        ...
        if (!config.chains.some(chain => chain.id === fromChainId) ||
            ...
            throw new errors_1.PathrSdkError('Not supported chain');
        }
        ...
        if (!supportedToToken) {
            throw new errors_1.PathrSdkError('Not supported tokens');
        }
    ...
    async getMinMaxAmountsErrors(fromToken, _feeInfo) {
        try {
            ...
            const fromTokenAddress = fromToken.isNative
                ? tokens_1.wrappedNativeTokensList[fromBlockchain].address
                : fromToken.address;
