...
const squidrouter_cross_chain_trade_1 = require("src/features/cross-chain/calculation-manager/providers/squidrouter-provider/squidrouter-cross-chain-trade");
...
class SquidrouterCrossChainProvider extends cross_chain_provider_1.CrossChainProvider {
    ...
    async calculate(from, toToken, options) {
        ...
        try {
            ...
            const { route: { transactionRequest, estimate } } = await injector_1.Injector.httpClient.get(`${SquidrouterCrossChainProvider.apiEndpoint}route`, {
            ...
            if (httpError?.message) {
                pathrSdkError = new errors_1.PathrSdkError(httpError.message);
            }
            return {
                trade: null,
                error: pathrSdkError,
