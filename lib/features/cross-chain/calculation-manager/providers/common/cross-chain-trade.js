...
const check_address_1 = require("src/features/common/utils/check-address");
/**
...
class CrossChainTrade {
    ...
    get networkFee() {
        return new bignumber_js_1.default(this.feeInfo.pathrProxy?.fixedFee?.amount || 0).plus(this.feeInfo.provider?.cryptoFee?.amount || 0);
    }
    get platformFee() {
        return new bignumber_js_1.default(this.feeInfo.pathrProxy?.platformFee?.percent || 0).plus(this.feeInfo.provider?.platformFee?.percent || 0);
    }
    get isProxyTrade() {
    ...
    async checkFromAddress(fromAddress, isRequired = false, crossChainType) {
        if (!fromAddress) {
            if (isRequired) {
                throw new errors_1.PathrSdkError(`'fromAddress' is required option`);
            }
            return;
        }
    ...
    async checkReceiverAddress(receiverAddress, isRequired = false, crossChainType) {
        if (!receiverAddress) {
            if (isRequired) {
                throw new errors_1.PathrSdkError(`'receiverAddress' is required option`);
            }
            return;
        }
    ...
    getSwapValue(providerValue) {
        ...
        const fixedFeeValue = web3_pure_1.Web3Pure.toWei(this.feeInfo.pathrProxy?.fixedFee?.amount || 0, nativeToken.decimals);
        let fromValue;
        if (this.from.isNative) {
            if (providerValue) {
                fromValue = new bignumber_js_1.default(providerValue).dividedBy(1 - (this.feeInfo.pathrProxy?.platformFee?.percent || 0) / 100);
            }
            else {
