...
const unapproved_contract_error_1 = require("src/common/errors/proxy/unapproved-contract-error");
const unapproved_method_error_1 = require("src/common/errors/proxy/unapproved-method-error");
...
const typed_trade_providers_1 = require("src/features/on-chain/calculation-manager/constants/trade-providers/typed-trade-providers");
const on_chain_manager_1 = require("src/features/on-chain/calculation-manager/on-chain-manager");
...
class ProxyCrossChainEvmTrade {
    static async getFeeInfo(fromBlockchain, providerAddress, percentFeeToken, useProxy) {
        const fixedFeeAmount = useProxy
            ? await ProxyCrossChainEvmTrade.getFixedFee(fromBlockchain, providerAddress, pathr_proxy_contract_address_1.pathrProxyContractAddress[fromBlockchain].router, evm_common_cross_chain_abi_1.evmCommonCrossChainAbi)
            : new bignumber_js_1.default(0);
        const feePercent = useProxy
            ? await ProxyCrossChainEvmTrade.getFeePercent(fromBlockchain, providerAddress, pathr_proxy_contract_address_1.pathrProxyContractAddress[fromBlockchain].router, evm_common_cross_chain_abi_1.evmCommonCrossChainAbi)
            : 0;
        const nativeToken = await tokens_1.PriceToken.createFromToken(tokens_1.nativeTokensList[fromBlockchain]);
        return {
            pathrProxy: {
                fixedFee: {
    ...
    static async getFeePercent(fromBlockchain, providerAddress, contractAddress, contractAbi) {
        ...
        return (new bignumber_js_1.default(await web3Public.callContractMethod(contractAddress, contractAbi, 'PathrPlatformFee')).toNumber() / 10000);
    }
        ...
        if (from.isNative) {
            try {
                const wrapToken = isCustomWeth
                    ? to.asStruct
                    : tokens_1.wrappedNativeTokensList[fromBlockchain];
    ...
    static async getWhitelistedDexes(fromBlockchain) {
        const web3Public = injector_1.Injector.web3PublicService.getWeb3Public(fromBlockchain);
        return web3Public.callContractMethod(pathr_proxy_contract_address_1.pathrProxyContractAddress[fromBlockchain].router, evm_common_cross_chain_abi_1.evmCommonCrossChainAbi, 'approvedDexs');
    }
    ...
    static async checkCrossChainWhiteList(fromBlockchain, routerAddress, offset) {
        const web3Public = injector_1.Injector.web3PublicService.getWeb3Public(fromBlockchain);
        const result = await web3Public.callContractMethod(pathr_proxy_contract_address_1.pathrProxyContractAddress[fromBlockchain].router, evm_common_cross_chain_abi_1.evmCommonCrossChainAbi, 'getSelectorInfo', [routerAddress, offset]);
    ...
    static async checkDexWhiteList(fromBlockchain, routerAddress, method) {
        ...
        try {
            isRouterApproved = await web3Public.callContractMethod(pathr_proxy_contract_address_1.pathrProxyContractAddress[fromBlockchain].router, evm_common_cross_chain_abi_1.evmCommonCrossChainAbi, 'isContractApproved', [routerAddress]);
        }
        catch { }
        if (!isRouterApproved) {
        ...
        try {
            isMethodApproved = await web3Public.callContractMethod(pathr_proxy_contract_address_1.pathrProxyContractAddress[fromBlockchain].router, evm_common_cross_chain_abi_1.evmCommonCrossChainAbi, 'isFunctionApproved', [method]);
        }
        catch { }
        if (!isMethodApproved) {
