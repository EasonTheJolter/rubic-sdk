...
class CrossChainProvider {
    ...
    async checkContractState(fromBlockchain, pathrRouter, contractAbi) {
        const web3PublicService = injector_1.Injector.web3PublicService.getWeb3Public(fromBlockchain);
        const isPaused = await web3PublicService.callContractMethod(pathrRouter, contractAbi, 'paused');
        if (isPaused) {
