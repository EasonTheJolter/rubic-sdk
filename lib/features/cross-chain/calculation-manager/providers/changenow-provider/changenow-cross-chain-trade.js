...
const options_1 = require("src/common/utils/options");
const blockchains_info_1 = require("src/core/blockchain/utils/blockchains-info/blockchains-info");
const erc_20_token_abi_1 = require("src/core/blockchain/web3-public-service/web3-public/evm-web3-public/constants/erc-20-token-abi");
...
class ChangenowCrossChainTrade extends evm_cross_chain_trade_1.EvmCrossChainTrade {
    ...
    static async getGasData(changenowTrade, providerAddress, receiverAddress) {
        ...
        try {
            const { contractAddress, contractAbi, methodName, methodArguments, value } = await new ChangenowCrossChainTrade(changenowTrade, providerAddress || evm_web3_pure_1.EvmWeb3Pure.EMPTY_ADDRESS, []).getContractParams({ receiverAddress: receiverAddress || walletAddress });
    ...
    get fromContractAddress() {
        if (this.isProxyTrade) {
            return pathr_proxy_contract_address_1.pathrProxyContractAddress[this.from.blockchain].gateway;
        }
        throw new errors_1.PathrSdkError('No contract address for changenow provider');
    }
    get web3Private() {
        if (!blockchains_info_1.BlockchainsInfo.isEvmBlockchainName(this.from.blockchain)) {
            throw new errors_1.PathrSdkError('Cannot retrieve web3 private');
        }
    ...
    async swapDirect(options = {}) {
        if (!blockchains_info_1.BlockchainsInfo.isEvmBlockchainName(this.from.blockchain)) {
            throw new errors_1.PathrSdkError("For non-evm chains use 'getChangenowPostTrade' method");
        }
        ...
        try {
            const { id, payinAddress } = await this.getPaymentInfo(this.transitToken.tokenAmount, options.receiverAddress ? options.receiverAddress : this.walletAddress);
            this.id = id;
    ...
    async getPaymentInfo(fromAmount, receiverAddress) {
        return injector_1.Injector.httpClient.post('https://api.changenow.io/v2/exchange', {
    ...
    async getContractParams(options) {
        const paymentInfo = await this.getPaymentInfo(this.transitToken.tokenAmount, options?.receiverAddress || this.walletAddress);
    ...
    async encode(options) {
        if (!blockchains_info_1.BlockchainsInfo.isEvmBlockchainName(this.from.blockchain)) {
            throw new errors_1.PathrSdkError('Cannot encode trade for non-evm blockchain');
        }
    ...
    encodeApprove() {
        throw new errors_1.PathrSdkError('Cannot encode approve for changenow');
    }
    async needApprove() {
        if (this.isProxyTrade) {
