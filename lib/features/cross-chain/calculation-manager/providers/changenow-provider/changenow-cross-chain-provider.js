...
const changenow_cross_chain_trade_1 = require("src/features/cross-chain/calculation-manager/providers/changenow-provider/changenow-cross-chain-trade");
const changenow_api_blockchain_1 = require("src/features/cross-chain/calculation-manager/providers/changenow-provider/constants/changenow-api-blockchain");
const changenow_proxy_supported_blockchains_1 = require("src/features/cross-chain/calculation-manager/providers/changenow-provider/constants/changenow-proxy-supported-blockchains");
const native_addresses_1 = require("src/features/cross-chain/calculation-manager/providers/changenow-provider/constants/native-addresses");
...
class ChangenowCrossChainProvider extends cross_chain_provider_1.CrossChainProvider {
    ...
    async calculate(from, toToken, options) {
        ...
        if (!fromCurrency) {
            ...
            onChainTrade = transitCurrency
                ...
                : null;
            if (!onChainTrade) {
                onChainTrade = await this.getOnChainTrade(fromWithoutFee, [], options.slippageTolerance, tokens_1.nativeTokensList[fromBlockchain].address);
            }
            if (!onChainTrade) {
                return {
                    trade: null,
    ...
    async getChangenowCurrencies(from, to) {
        const currencies = await injector_1.Injector.httpClient.get('https://api.changenow.io/v2/exchange/currencies?active=true&flow=standard', { headers: { 'x-changenow-api-key': changenow_api_key_1.changenowApiKey } });
        const nativeToken = tokens_1.nativeTokensList[from.blockchain];
        return {
    ...
    async getToAmount(fromCurrency, toCurrency, fromAmount) {
        const response = await injector_1.Injector.httpClient.get(`https://api.changenow.io/v2/exchange/estimated-amount?flow=standard`, {
            params: {
            ...
            headers: {
                'x-changenow-api-key': changenow_api_key_1.changenowApiKey
            }
        });
        return new bignumber_js_1.default(response.toAmount);
    }
    async getMinMaxRange(fromCurrency, toCurrency) {
        const response = await injector_1.Injector.httpClient.get(`https://api.changenow.io/v2/exchange/range?flow=standard`, {
            params: {
                fromCurrency: fromCurrency.ticker,
                toCurrency: toCurrency.ticker,
                fromNetwork: fromCurrency.network,
