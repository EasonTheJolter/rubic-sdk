    ...
    static async getGasData(from, to, receiverAddress, providerAddress, feeInfo, toTokenAmountMin) {
        ...
        try {
            ...
            if (feeInfo.pathrProxy?.fixedFee?.amount.gt(0)) {
                const { contractAddress, contractAbi, methodName, methodArguments, value } = await new EvmBridgersCrossChainTrade({
                    from,
                    to,
                    ...
                    slippage: 0,
                    contractAddress: ''
                }, providerAddress || evm_web3_pure_1.EvmWeb3Pure.EMPTY_ADDRESS, []).getContractParams({ receiverAddress });
                ...
                const { transactionData } = await (0, get_method_arguments_and_transaction_data_1.getMethodArgumentsAndTransactionData)(from, fromWithoutFee, to, toTokenAmountMin, walletAddress, providerAddress, { receiverAddress, fromAddress: walletAddress });
