...
const check_unsupported_receiver_address_1 = require("src/features/common/utils/check-unsupported-receiver-address");
...
const relayers_addresses_1 = require("src/features/cross-chain/calculation-manager/providers/stargate-provider/constants/relayers-addresses");
const stargate_bridge_token_1 = require("src/features/cross-chain/calculation-manager/providers/stargate-provider/constants/stargate-bridge-token");
const stargate_pool_id_1 = require("src/features/cross-chain/calculation-manager/providers/stargate-provider/constants/stargate-pool-id");
const stargate_pools_decimals_1 = require("src/features/cross-chain/calculation-manager/providers/stargate-provider/constants/stargate-pools-decimals");
const stargate_cross_chain_provider_1 = require("src/features/cross-chain/calculation-manager/providers/stargate-provider/stargate-cross-chain-provider");
...
class StargateCrossChainTrade extends evm_cross_chain_trade_1.EvmCrossChainTrade {
    ...
    static async getGasData(from, toToken, feeInfo, srcChainTrade, dstChainTrade, slippageTolerance, providerAddress, receiverAddress) {
        ...
        try {
            ...
            if (feeInfo.pathrProxy?.fixedFee?.amount.gt(0)) {
                ...
                const lzFeeWei = web3_pure_1.Web3Pure.toWei(feeInfo.provider.cryptoFee.amount, tokens_1.nativeTokensList[from.blockchain].decimals);
    ...
    async swapDirect(options = {}) {
        ...
        try {
            ...
            const lzFeeWei = web3_pure_1.Web3Pure.toWei(this.feeInfo.provider.cryptoFee.amount, tokens_1.nativeTokensList[this.from.blockchain].decimals);
    ...
    static async getLayerZeroSwapData(from, to, tokenAmountMin = to.stringWeiAmount, receiverAddress, dstData) {
        ...
        const fromSymbol = stargate_cross_chain_provider_1.StargateCrossChainProvider.getSymbol(from.symbol, fromBlockchain);
        const toSymbol = stargate_cross_chain_provider_1.StargateCrossChainProvider.getSymbol(to.symbol, toBlockchain);
    ...
    async getContractParams(options) {
        ...
        const swapData = this.onChainTrade &&
            (await proxy_cross_chain_evm_trade_1.ProxyCrossChainEvmTrade.getSwapData(options, {
                walletAddress: this.walletAddress,
                contractAddress: pathr_proxy_contract_address_1.pathrProxyContractAddress[this.from.blockchain].router,
                fromTokenAmount: this.from,
        ...
        const lzWeiFee = web3_pure_1.Web3Pure.toWei(this.feeInfo.provider.cryptoFee.amount, tokens_1.nativeTokensList[this.from.blockchain].decimals);
    ...
    getProviderData(_sourceData, dstSwapData, receiverAddress) {
        const toSymbol = stargate_cross_chain_provider_1.StargateCrossChainProvider.getSymbol(this.to.symbol, this.to.blockchain);
        ...
        const fee = web3_pure_1.Web3Pure.toWei(this.feeInfo.provider.cryptoFee.amount, tokens_1.nativeTokensList[this.from.blockchain].decimals);
