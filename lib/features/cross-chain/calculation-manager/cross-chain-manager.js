...
const proxy_supported_blockchain_1 = require("src/features/common/constants/proxy-supported-blockchain");
const get_price_tokens_from_input_tokens_1 = require("src/features/common/utils/get-price-tokens-from-input-tokens");
const cross_chain_providers_1 = require("src/features/cross-chain/calculation-manager/constants/cross-chain-providers");
const default_cross_chain_calculation_options_1 = require("src/features/cross-chain/calculation-manager/constants/default-cross-chain-calculation-options");
...
const compare_cross_chain_trades_1 = require("src/features/cross-chain/calculation-manager/utils/compare-cross-chain-trades");
/**
...
class CrossChainManager {
    ...
    async calculateTrade(fromToken, fromAmount, toToken, options) {
        if (toToken instanceof tokens_1.Token && fromToken.blockchain === toToken.blockchain) {
            throw new errors_1.PathrSdkError('Blockchains of `from` and `to` tokens must be different.');
        }
        ...
        if (!wrappedTrades?.length) {
            throw new errors_1.PathrSdkError('No success providers calculation for the trade');
        }
    ...
    getNotDisabledProviders(fromBlockchain, toBlockchain, disabledProviders) {
        ...
        if (!providers.length) {
            throw new errors_1.PathrSdkError(`There are no providers for trade`);
        }
        return providers;
    }
