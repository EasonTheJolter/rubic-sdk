...
const limit_order_api_service_1 = require("src/features/limit-order/limit-order-api-service");
const supported_blockchains_1 = require("src/features/limit-order/models/supported-blockchains");
const get_parsed_token_amounts_1 = require("src/features/limit-order/utils/get-parsed-token-amounts");
class LimitOrderManager {
    ...
    async createOrder(fromToken, toToken, fromAmount, toAmount, options) {
        ...
        if (fromTokenAmount.blockchain !== toTokenAmount.blockchain) {
            throw new errors_1.PathrSdkError('Blockchains must be equal');
        }
        this.checkWalletConnected();
        const { blockchain } = fromToken;
        await this.web3Private.checkBlockchainCorrect(blockchain);
    ...
    async getCancelCallData(blockchain, orderHash) {
        ...
        if (!order) {
            throw new errors_1.PathrSdkError(`No order with hash ${orderHash}`);
        }
        const chainId = blockchain_id_1.blockchainId[blockchain];
        const connector = new limit_order_protocol_utils_1.Web3ProviderConnector(this.web3Private.web3);
