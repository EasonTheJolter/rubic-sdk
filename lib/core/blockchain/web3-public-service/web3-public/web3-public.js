...
const multicall_addresses_1 = require("src/core/blockchain/web3-public-service/web3-public/constants/multicall-addresses");
...
class Web3Public {
    ...
    async callForTokensInfo(tokenAddresses, tokenFields = ['decimals', 'symbol', 'name']) {
        ...
        const tokens = results.map((tokenFieldsResults, tokenIndex) => {
            ...
            return tokenFieldsResults.reduce((acc, field, fieldIndex) => {
                if (!field.success) {
                    throw new errors_1.PathrSdkError(`Cannot retrieve information about ${tokenAddress}`);
                }
                return {
                    ...acc,
    ...
    async multicallContractMethods(contractAddress, contractAbi, methodsData) {
        ...
        if (!results?.[0]) {
            throw new errors_1.PathrSdkError('Cant perform multicall or request data is empty');
        }
        return results[0];
    }
}
